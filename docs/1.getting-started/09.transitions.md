---
title: 'Переходы'
description: Переходы между страницами и лейаутами через Vue или нативный View Transitions API браузера.
...
Вы можете включить переходы между страницами, чтобы применить автоматический переход для всех ваших [страниц](/docs/3.x/directory-structure/pages/).
...
::note
Если вы меняете лейауты, а также страницу, переход страницы, который вы здесь задали, не будет запущен. Вместо этого следует задать [переход лейаута](/docs/3.x/getting-started/transitions#layout-transitions).
::

Чтобы начать добавлять переходы между страницами, добавьте следующий CSS в ваш [`app.vue`](/docs/3.x/directory-structure/app/):
...
Так выглядит результат при переходе между страницами:
...
Чтобы задать другой переход для страницы, установите ключ `pageTransition` в [`definePageMeta`](/docs/3.x/api/utils/define-page-meta) страницы:
...
Вы можете включить переходы лейаутов, чтобы применить автоматический переход для всех [лейаутов](/docs/3.x/directory-structure/layouts/).
...
Чтобы включить переходы лейаутов, добавьте в [`app.vue`](/docs/3.x/directory-structure/app/) такой CSS:
...
Так выглядит результат при смене лейаутов:
...
Чтобы применить динамические переходы с использованием условной логики, вы можете использовать встроенные [посредники маршрута](/docs/3.x/directory-structure/middleware/), чтобы назначить другое имя перехода для `to.meta.pageTransition`.
...
definePageMeta({
  ...
  middleware (to, from) {
    if (to.meta.pageTransition && typeof to.meta.pageTransition !== 'boolean') { to.meta.pageTransition.name = +to.params.id! > +from.params.id! ? 'slide-left' : 'slide-right' }
  },
})
</script>
...
Интеграция в Nuxt активно развивается; включите её опцией `experimental.viewTransition` в конфиге:
...
При `true` Nuxt отключает переходы при `prefers-reduced-motion: reduce` (так рекомендуется). При `always` переходы всегда включены — забота о доступности остаётся на вас.

По умолчанию переходы между представлениями включены для всех [страниц](/docs/3.x/directory-structure/pages/), но вы можете установить другое глобальное значение по умолчанию.
...
Можно переопределить значение `viewTransition` по умолчанию для страницы, установив ключ `viewTransition` в [`definePageMeta`](/docs/3.x/api/utils/define-page-meta) страницы:
