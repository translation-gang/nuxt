...
::important
Nuxt использует компонент Vue `<Transition>`, поэтому страница или макет, которые вы хотите анимировать, должны иметь **один корневой элемент**. Страница или макет с несколькими корнями (фрагмент) не анимируются: переход не сработает, а навигация между маршрутами может вызвать ошибку. В режиме разработки Nuxt предупреждает об этом. Оберните шаблон в один корневой элемент (например, `<div>`).
::
