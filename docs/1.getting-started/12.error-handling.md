...
В режиме разработки `cause` ошибки сохраняется и попадает на страницу ошибки, чтобы вы могли отследить исходную причину. В production `cause` не включается ни в ответы об ошибках, ни в payload страницы ошибки.
