...
### Статические fallback-страницы

Nuxt может сгенерировать две fallback-страницы для статического хостинга:

- `200.html` — fallback для SPA. Настройте хостинг так, чтобы для несовпадающих маршрутов отдавался этот файл, если маршрутизацию обрабатывает клиент.
- `404.html` — fallback для страницы «не найдено». Настройте хостинг так, чтобы для таких маршрутов сохранялся статус 404.

`nuxt generate` и `nuxt build --prerender` создают эти файлы автоматически. Если вы используете `nuxt build` с route rules для пререндеринга отдельных маршрутов, добавьте fallback-страницу явно:
...
Один провайдер использует `200.html`, другой — `404.html`, третий позволяет настроить оба варианта. После деплоя проверьте настройки fallback или rewrite у вашего хостинга.
