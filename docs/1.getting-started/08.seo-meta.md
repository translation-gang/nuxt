...
::note
Если задан [`app.cdnURL`](/docs/4.x/api/nuxt-config#cdnurl), ресурсы из [`public/`](/docs/4.x/directory-structure/public) (включая `favicon.ico`) отдаются с CDN. Nuxt разрешает пути к public-ресурсам через `cdnURL`, с запасным вариантом `app.baseURL`. Статическая ссылка в `app.head`, например `href: '/favicon.ico'`, — это буквальный путь и **не** разрешается через `cdnURL`. Чтобы favicon указывал на правильный URL, соберите `href` из runtime config через [`useHead()`](/docs/4.x/api/composables/use-head) в `app.vue`:
