...
::note
Nuxt leverages Vue's [`<Transition>`](https://ru.vuejs.org/guide/built-ins/transition.html#the-transition-component) component to apply transitions between pages and layouts.
::
...
Both `pageTransition` and `layoutTransition` keys accept [`TransitionProps`](https://ru.vuejs.org/api/built-in-components.html#transition) as JSON serializable values where you can pass the `name`, `mode` and other valid transition-props of the custom CSS transition.
...
::tip
Learn more about additional [JavaScript hooks](https://ru.vuejs.org/guide/built-ins/transition.html#javascript-hooks) available in the `Transition` component.
::
