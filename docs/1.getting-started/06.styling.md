...
Nuxt гибок в части стилей: собственный CSS, локальные и внешние таблицы, препроцессоры, CSS-фреймворки, UI-библиотеки и модули Nuxt.
...
Можно подключить стили через импорт в JavaScript или [директиву `@import`](https://developer.mozilla.org/en-US/docs/Web/CSS/@import) в CSS.
...
// Внимание: динамический импорт несовместим с серверным рендерингом.
import('~/assets/css/first.css')
</script>
...
В Vite есть [экспериментальная опция](https://vite.dev/config/shared-options.html#css-preprocessormaxworkers), ускоряющая препроцессоры.
...
export default defineNuxtConfig({
  vite: {
    css: {
      preprocessorMaxWorkers: true // обычно число CPU минус 1
    }
  }
})
```
...
Модули Nuxt дают удобный опыт разработки из коробки, но отсутствие модуля не мешает подключить инструмент вручную. Часто достаточно [плагина Nuxt](/docs/3.x/directory-structure/plugins/) и/или [собственного модуля](/docs/3.x/guide/going-further/modules). Если сделаете что-то полезное — поделитесь с [сообществом](/modules).
