...
Модули Nuxt дают удобный DX из коробки, но отсутствие модуля не мешает подключить инструмент вручную. Часто достаточно [плагина Nuxt](/docs/3.x/directory-structure/plugins) и/или [собственного модуля](/docs/3.x/guide/going-further/modules). Если сделаете что-то полезное — поделитесь с [сообществом](/modules).
