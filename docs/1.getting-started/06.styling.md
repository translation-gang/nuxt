...
Если вы пишете локальные таблицы стилей, стандартным местом для их размещения является [директория `assets/`](/docs/3.x/directory-structure/assets).

### Импорт внутри компонентов

Вы можете импортировать таблицы стилей напрямую в свои страницы, лейауты и компоненты.
Можно подключить стили через импорт в JavaScript или [директиву `@import`](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/At-rules/@import) в CSS.
...
Стандартным местом для ваших таблиц стилей является [директория `assets/`](/docs/3.x/directory-structure/assets). Вы можете указать путь к файлу стилей, и Nuxt включит его во все страницы приложения.
...
Вы можете управлять элементом head с помощью свойства [`app.head`](/docs/3.x/api/nuxt-config#head) конфигурации Nuxt:
...
<!-- TODO: разобраться, как через twoslash подставить типы для другого контекста -->
...
Внешние таблицы стилей являются блокирующими рендеринг ресурсами: они должны быть загружены и обработаны до того, как браузер отобразит страницу. Веб-страницы, содержащие неоправданно большие стили, требуют больше времени для рендеринга. Подробнее об этом можно прочитать на [web.dev](https://web.dev/articles/defer-non-critical-css).
...
Если вам необходимо внедрить код в предварительно обработанные файлы, такие как [Sass-фрагменты](https://sass-lang.com/documentation/at-rules/use/#partials) с переменными цветов, вы можете сделать это с помощью [параметров препроцессоров](https://vite.dev/config/shared-options#css-preprocessoroptions) Vite.
...
::note
Опция экспериментальная; детали — в документации Vite, обратную связь можно оставить [в обсуждении](https://github.com/vitejs/vite/discussions/15835).
::
...
Вы можете обратиться к [документации Vite CSS](https://vite.dev/guide/features#css) и [документации @vitejs/plugin-vue](https://github.com/vitejs/vite-plugin-vue/tree/main/packages/plugin-vue).
...
Nuxt не навязывает один способ стилизации — доступен широкий выбор инструментов, например [UnoCSS](https://unocss.dev) или [Tailwind CSS](https://tailwindcss.com).

Сообщество и команда выпускают модули, упрощающие интеграцию; каталог — в разделе [модули](/modules). Несколько примеров для старта:
...
Модули Nuxt дают удобный опыт разработки из коробки, но отсутствие модуля не мешает подключить инструмент вручную. Часто достаточно [плагина Nuxt](/docs/3.x/directory-structure/plugins) и/или [собственного модуля](/docs/3.x/guide/modules). Если сделаете что-то полезное — поделитесь с [сообществом](/modules).
...
Nuxt поставляется с тем же компонентом `<Transition>`, что и Vue, а также поддерживает экспериментальный [View Transitions API](/docs/3.x/getting-started/transitions#view-transitions-api-experimental).

:read-more{to="/docs/3.x/getting-started/transitions"}

### Продвинутая оптимизация шрифтов

Мы рекомендуем использовать [Fontaine](https://github.com/nuxt-modules/fontaine) для сокращения вашего [CLS](https://web.dev/articles/cls). Если вам нужно что-то более продвинутое, рассмотрите возможность создания модуля Nuxt для расширения процесса сборки или рантайма Nuxt.
...
Вы можете найти руководство по оптимизации LCP на [web.dev](https://web.dev/articles/optimize-lcp).
