...
Функция `execute` - это псевдоним для `refresh`, который работает точно так же, но является более семантичным для случаев, когда выборка происходит [не срочно](#не-срочно).
...
При каждом изменении зависимости данные будут извлекаться по новому построенному URL. В сочетании с [не срочной опцией](#не-срочно) вы можете подождать, пока реактивный элемент не изменится, прежде чем выполнять получение данных.
