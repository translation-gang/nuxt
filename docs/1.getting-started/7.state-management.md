...
[`useState`](/docs/api/composables/use-state) is an SSR-friendly [`ref`](https://ru.vuejs.org/api/reactivity-core.html#ref) replacement. Its value will be preserved after server-side rendering (during client-side hydration) and shared across all components using a unique key.
