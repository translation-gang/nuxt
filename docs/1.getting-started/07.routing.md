...
::note
Code-splitting включён по умолчанию и рекомендуется для большинства приложений. Если вам нужен один общий бандл, отключите его в [`nuxt.config`](/docs/4.x/api/nuxt-config):
...
Весь JavaScript попадёт в один файл. Обычно это ухудшает первую загрузку, даже на медленных соединениях. Отключайте code-splitting только если замеры показали пользу для вашего случая.
::
