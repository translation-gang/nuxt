...
Одна из ключевых возможностей Nuxt — поддержка слоёв и расширения. Можно расширять приложение Nuxt по умолчанию и переиспользовать компоненты, утилиты и конфигурацию. Структура слоя почти совпадает со структурой обычного Nuxt-приложения, поэтому слои удобно писать и поддерживать.
...
layers/
  ...
  2.theme/
    components/Button.vue    # Тематическая кнопка (перекрывает base)
app/
