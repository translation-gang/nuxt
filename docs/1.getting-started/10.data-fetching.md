...
- `clear`: функция, которая может быть использована для установки `data` в `undefined` (or the value of `options.default()` if provided), установки `error` в `null`, установки `pending` в `false`, установки `status` в `idle` и пометки всех текущих запросов как отмененных.
