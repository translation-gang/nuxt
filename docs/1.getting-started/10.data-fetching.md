...
::note
Если вы не получили данные на сервере (например, с `server: false`), данные _не_ загрузятся до завершения гидратации. Даже при `await useFetch` на клиенте в `<script setup>` значение `data` останется `undefined`.
::
