...
Nuxt поставляется с двумя композаблами и встроенной библиотекой для получения данных в браузере или на сервере: `useFetch`, [`useAsyncData`](/docs/3.x/api/composables/use-async-data) и `$fetch`.

В двух словах:

- [`$fetch`](/docs/3.x/api/utils/dollarfetch) — простейший способ выполнить HTTP-запрос.
- [`useFetch`](/docs/3.x/api/composables/use-fetch) — обёртка над `$fetch`, которая при [универсальном рендеринге](/docs/3.x/guide/concepts/rendering#universal-rendering) запрашивает данные один раз.
- [`useAsyncData`](/docs/3.x/api/composables/use-async-data) похож на `useFetch`, но даёт более тонкий контроль.
...
Nuxt выполняет изоморфный (универсальный) код и на сервере, и в браузере. Если в `setup` компонента вызывать только [`$fetch`](/docs/3.x/api/utils/dollarfetch), запрос может уйти дважды: при SSR для HTML и снова при гидратации на клиенте — отсюда риски рассинхрона, лишняя задержка до интерактива и непредсказуемое поведение.

Композаблы [`useFetch`](/docs/3.x/api/composables/use-fetch) и [`useAsyncData`](/docs/3.x/api/composables/use-async-data) решают это: если данные уже получены на сервере, они попадают в полезную нагрузку (`payload`) и на клиенте повторный запрос не делается.

Полезная нагрузка — это объект JavaScript, доступный через [`useNuxtApp().payload`](/docs/3.x/api/composables/use-nuxt-app#payload). Он используется на клиенте, чтобы избежать повторного запроса одних и тех же данных при выполнении кода в браузере [во время гидратации](/docs/3.x/guide/concepts/rendering#universal-rendering).

::tip
Используйте [Nuxt DevTools](https://devtools.nuxt.com): на вкладке **Payload** видна сериализованная полезная нагрузка.
::
...
Под капотом Nuxt использует [`<Suspense>`](https://vuejs.org/guide/built-ins/suspense) Vue, чтобы не завершать навигацию, пока асинхронные данные для экрана не готовы. Композаблы загрузки данных подстраиваются под это поведение; конкретные опции задаются на каждый вызов.

::note
Можно добавить [`<NuxtLoadingIndicator>`](/docs/3.x/api/components/nuxt-loading-indicator), чтобы показывать полосу прогресса между переходами по страницам.
::
...
::warning
Помните, что использование только `$fetch` не обеспечит [дедупликацию сетевых вызовов и предотвращение навигации](/docs/3.x/getting-started/data-fetching#the-need-for-usefetch-and-useasyncdata). :br
Рекомендуется использовать `$fetch` для взаимодействия на стороне клиента (event-based) или в сочетании с [`useAsyncData`](/docs/3.x/getting-started/data-fetching#useasyncdata) при получении исходных данных компонента.
::
...
При вызове `useFetch` на сервере Nuxt использует [`useRequestFetch`](/docs/3.x/api/composables/use-request-fetch) и проксирует заголовки и куки клиента (кроме тех, что не предназначены для проброса, например `host`).
...
Ниже — как вручную взять [`useRequestHeaders`](/docs/3.x/api/composables/use-request-headers) и передать куки в API с серверной части запроса, пришедшего от клиента. Изоморфный `$fetch` с теми же заголовками даёт эндпоинту тот же `cookie`, что и у браузера. Это нужно, если вы не используете `useFetch`.
...
::tip
Также можно вызвать [`useRequestFetch`](/docs/3.x/api/composables/use-request-fetch) — заголовки к запросу подставятся автоматически.
::
...
Композабл [`useFetch`](/docs/3.x/api/composables/use-fetch) использует `$fetch` под капотом для выполнения SSR-безопасных сетевых вызовов в функции setup.
...
Этот композабл — обёртка над [`useAsyncData`](/docs/3.x/api/composables/use-async-data) и утилитой `$fetch`.
...
Это сахар для типичных сценариев разработки. Подробнее о `event.fetch` см. в [`useRequestFetch`](/docs/3.x/api/composables/use-request-fetch).
::
...
Бывают случаи, когда использование композабла [`useFetch`](/docs/3.x/api/composables/use-fetch) не подходит, например, когда CMS или сторонние разработчики предоставляют свой собственный слой запросов. В этом случае вы можете использовать [`useAsyncData`](/docs/3.x/api/composables/use-async-data), чтобы обернуть ваши вызовы и сохранить преимущества, предоставляемые композаблами.
...
::note
Первый аргумент [`useAsyncData`](/docs/3.x/api/composables/use-async-data) — уникальный ключ для кэширования ответа второго аргумента (функции запроса). Этот ключ можно игнорировать, передавая напрямую функцию запроса, ключ будет сгенерирован автоматически.
:br :br
...
:br :br
Установка ключа может быть полезна для обмена одними и теми же данными между компонентами с помощью [`useNuxtData`](/docs/3.x/api/composables/use-nuxt-data) или для [обновления выбранных данных](/docs/3.x/api/utils/refresh-nuxt-data#refresh-specific-data).
::
...
::note
`useAsyncData` предназначен для загрузки и кэширования данных, а не для побочных эффектов вроде вызова экшенов Pinia — иначе возможны повторные запуски и странное поведение с пустыми значениями. Для побочных эффектов используйте [`callOnce`](/docs/3.x/api/utils/call-once).
...
::note
Если вы не получили данные на сервере (например, с помощью `server: false`), то данные _не_ будут получены до завершения гидратации. Это означает, что даже если вы ожидаете `useFetch` на стороне клиента, `data` останется `undefined` внутри `<script setup>`.
::

## Параметры

[`useAsyncData`](/docs/3.x/api/composables/use-async-data) и [`useFetch`](/docs/3.x/api/composables/use-fetch) возвращают один и тот же тип объекта и принимают общий набор опций в качестве последнего аргумента. С их помощью можно управлять поведением композаблов, например, блокировкой навигации, кэшированием или выполнением.
...
В качестве альтернативы вы можете использовать [`useLazyFetch`](/docs/3.x/api/composables/use-lazy-fetch) и `useLazyAsyncData` как удобные методы для выполнения того же самого.
...
:video-accordion{title="Vue School: блокирующие и отложенные (lazy) запросы" videoId="1022000555" platform="vimeo"}
...
Композабл `useFetch` предназначен для вызова в методе `setup` или непосредственно на верхнем уровне функции в хуках жизненного цикла, в противном случае следует использовать метод [`$fetch`](/docs/3.x/getting-started/data-fetching#fetch).
...
:video-accordion{title="Vue School: уменьшение размера полезной нагрузки" videoId="1026410430" platform="vimeo"}

### Кэширование и повторное получение данных

#### Ключи

[`useFetch`](/docs/3.x/api/composables/use-fetch) и [`useAsyncData`](/docs/3.x/api/composables/use-async-data) используют ключи для предотвращения повторного запроса одних и тех же данных.

- [`useFetch`](/docs/3.x/api/composables/use-fetch) использует переданный URL в качестве ключа. Альтернативно ключ можно задать в объекте `options` последнего аргумента.
- [`useAsyncData`](/docs/3.x/api/composables/use-async-data) использует свой первый аргумент в качестве ключа, если он является строкой. Если первым аргументом является функция-обработчик, выполняющая запрос, то для вас будет сгенерирован ключ, уникальный для имени файла и номера строки экземпляра `useAsyncData`.

::tip
Чтобы получить кэшированные данные по ключу, вы можете использовать [`useNuxtData`](/docs/3.x/api/composables/use-nuxt-data).
::

:video-accordion{title="Vue School: кэширование данных и опция key" videoId="1026410044" platform="vimeo"}
...
`execute` — псевдоним `refresh` с тем же поведением; удобнее, когда выборка [не выполняется сразу](/docs/3.x/getting-started/data-fetching#not-immediate).

::tip
Для глобального повторного запроса или инвалидации кэшированных данных см. [`clearNuxtData`](/docs/3.x/api/utils/clear-nuxt-data) и [`refreshNuxtData`](/docs/3.x/api/utils/refresh-nuxt-data).
::
...
Если вам нужно изменить URL на основе реактивного значения, вместо него лучше использовать [вычисляемый URL](/docs/3.x/getting-started/data-fetching#computed-url).
...
В случае более сложного построения URL можно использовать обратный вызов в качестве [вычисляемого геттера](https://vuejs.org/guide/essentials/computed), который возвращает строку URL.

При каждом изменении зависимости данные будут извлекаться по новому построенному URL. В сочетании с [режимом без немедленного запроса](/docs/3.x/getting-started/data-fetching#not-immediate) вы можете дождаться изменения реактивного значения перед первой загрузкой.
...
Если вам нужно принудительно обновлять данные при изменении других реактивных значений, вы также можете [наблюдать за другими значениями](/docs/3.x/getting-started/data-fetching#watch).
...
Если же на сервере вызывается `useFetch` с относительным URL, Nuxt подставляет [`useRequestFetch`](/docs/3.x/api/composables/use-request-fetch) и проксирует заголовки и куки (кроме не предназначенных для проброса, например `host`).
...
При использовании `useAsyncData` и `useLazyAsyncData` для передачи данных, полученных на сервере, клиенту (а также всего остального, что использует [полезную нагрузку Nuxt](/docs/3.x/api/composables/use-nuxt-app#payload)), полезная нагрузка сериализуется с [`devalue`](https://github.com/sveltejs/devalue). Это позволяет нам передавать не только базовый JSON, но и сериализовывать и «оживить»/десериализовывать более сложные виды данных, такие как регулярные выражения, даты, Map и Set, `ref`, `reactive`, `shallowRef`, `shallowReactive` и `NuxtError` — и многое другое.

Также можно определить свой собственный сериализатор/десериализатор для типов, которые не поддерживаются Nuxt. Подробнее об этом можно прочитать в документации [`useNuxtApp`](/docs/3.x/api/composables/use-nuxt-app#payload).

::note
Обратите внимание: это _не относится_ к данным из ваших серверных маршрутов при запросах через `$fetch` или `useFetch` — подробности в следующем разделе.
::

## Сериализация данных из маршрутов API

При получении данных из директории `server` ответ сериализуется с помощью `JSON.stringify`. Однако, поскольку сериализация ограничена только примитивными типами JavaScript, Nuxt делает все возможное, чтобы преобразовать возвращаемый тип `$fetch` и [`useFetch`](/docs/3.x/api/composables/use-fetch) для соответствия реальному значению.
...
В примере ниже мы используем [superjson](https://github.com/flightcontrolhq/superjson) в качестве сериализатора.
...
::tip
Если вы используете SSE через GET-запрос, вы можете использовать [`EventSource`](https://developer.mozilla.org/en-US/docs/Web/API/EventSource) или композабл VueUse [`useEventSource`](https://vueuse.org/core/useeventsource/).
::
