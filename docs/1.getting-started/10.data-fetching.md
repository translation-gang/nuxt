...
В примерах документации вызовы `useFetch` и `useAsyncData` обычно сопровождаются `await`, но это не обязательно.

`await` **не** меняет HTML серверного рендера. При SSR Nuxt в любом случае ждёт завершения запроса перед сериализацией страницы (через `<Suspense>` и `onServerPrefetch`), поэтому в браузер уходит уже заполненный результат.

`await` влияет на то, что происходит дальше в вашем `<script setup>`, и на клиентскую навигацию:

- **С `await`** выполнение останавливается, пока данные не готовы: код после вызова может опираться на заполненный `data`. При клиентской навигации переход блокируется до завершения запроса: пользователь остаётся на текущей странице (опционально с [`<NuxtLoadingIndicator>`](/docs/4.x/api/components/nuxt-loading-indicator)) и попадает на страницу с готовыми данными. Так работает поведение по умолчанию.
- **Без `await`** выполнение идёт сразу, запрос идёт в фоне: `data` сначала имеет значение по умолчанию и заполняется после ответа. При клиентской навигации переход происходит сразу, а состояния загрузки и ошибки вы обрабатываете сами, обычно через ref `status` и `error`.

Универсально лучшего варианта нет: выбирайте под UX нужного маршрута.

Отказ от `await` даёт похожий эффект, что и опция [`lazy`](#lazy) (навигация не блокируется, загрузку обрабатываете сами), но это не одно и то же: `lazy` явно откладывает запрос до монтирования компонента, а отсутствие `await` запускает запрос уже в setup. Для неблокирующего поведения предпочитайте `lazy` (или [`useLazyFetch`](/docs/4.x/api/composables/use-lazy-fetch) / [`useLazyAsyncData`](/docs/4.x/api/composables/use-lazy-async-data)): намерение видно в коде.

::warning
`await` и `lazy` независимы: на клиенте `await` у lazy-вызова не даст ожидаемого эффекта. Если вы `await`-ите lazy-вызов (например `await useLazyFetch(...)` или `await useFetch(..., { lazy: true })`), SSR по-прежнему блокируется как обычно, но при **клиентской навигации** `await` завершится сразу, не дожидаясь запроса. Сразу после `await` `data` останется со значением по умолчанию, загрузку нужно обрабатывать через `status`. Чтобы навигация ждала данные, уберите `lazy`, а не полагайтесь на `await`.
::
...
::note
Если данные не запрашивались на сервере (например, при `server: false`), они _не будут_ загружены до завершения гидрации. Даже при await `useFetch` на клиенте `data` останется `undefined` внутри `<script setup>`.
::
...
- [`useFetch`](/docs/4.x/api/composables/use-fetch) генерирует ключ из URL, опций fetch и места вызова в исходном коде. Два вызова `useFetch` с одним URL в разных компонентах получат **разные** ключи и выполнят отдельные запросы. Чтобы разделить данные между компонентами, передайте одинаковый явный `key` в объекте опций последним аргументом.
- [`useAsyncData`](/docs/4.x/api/composables/use-async-data) использует первый аргумент как ключ, если это строка. Если первым аргументом передана функция-обработчик запроса, ключ, уникальный для места вызова `useAsyncData` в исходном коде, сгенерируется автоматически.
