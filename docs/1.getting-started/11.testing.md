...
> **Примечание**: `mockNuxtImport` можно использовать для мока импорта только один раз на тестовый файл. На самом деле это макрос, который преобразуется в `vi.mock`, а `vi.mock` поднимается, как описано [здесь](https://vitest.dev/api/vi.html#vi-mock).
...
> **Примечание**: Вы не можете ссылаться на локальные переменные в фабричной функции, так как они подняты. Если вам нужно получить доступ к API Vue или другим переменным, вам нужно импортировать их в фабричную функцию.
...
- `port`: Если указано, устанавливает порт запущенного тестового сервера на значение.
...
- `browser`: Под капотом Nuxt Test Utils использует [`playwright`](https://playwright.dev) для проведения тестирования в браузере. Если эта опция установлена, браузер будет запущен и может управляться в последующем наборе тестов.
  - Тип: `boolean`
  - По умолчанию: `false`
