...
Вы можете использовать javascript-импорт, или [css оператор `@import`](https://developer.mozilla.org/en-US/docs/Web/CSS/@import).
...
Если вам необходимо внедрить код в предварительно обработанные файлы, такие как [sass партиал](https://sass-lang.com/documentation/at-rules/use#partials) с переменными цветов, вы можете сделать это с помощью [параметров препроцессоров](https://vitejs.dev/config/shared-options.html#css-preprocessoroptions) Vite.
