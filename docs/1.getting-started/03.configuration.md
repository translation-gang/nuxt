...
export default defineNuxtConfig({
  ...
  $env: {
    staging: {
      // Optional overrides for staging
    }
  },
})
```
...
export default defineNuxtConfig({
  runtimeConfig: {
    ...
    apiSecret: '123',
    // Keys under public are also exposed to the client
    public: {
      apiBase: '/api'
    }
  }
...
Эти переменные станут доступны для всего приложения с помощью композабла [`useRuntimeConfig()`](/docs/3.x/api/composables/use-runtime-config).
...
Эти переменные станут доступны для всего остального приложения с помощью композабла [`useAppConfig`](/docs/3.x/api/composables/use-app-config).
