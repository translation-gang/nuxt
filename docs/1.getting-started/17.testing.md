...
3. Если тесты в окружении Nuxt лежат вне `test/nuxt/`, добавьте их в контекст TypeScript — см. [Поддержка TypeScript в тестах](#typescript-support-in-tests).
...
По умолчанию `false` — используется [`fake-indexeddb`](https://github.com/dumbmatter/fakeIndexedDB) для рабочего мока IndexedDB API.
...
Объект опций принимает опции монтирования из `@vue/test-utils` и следующие свойства:

  - `route`: начальный маршрут или `false`, чтобы пропустить начальную смену маршрута (по умолчанию `/`).
...
Объект опций принимает опции рендера из `@testing-library/vue` и следующие свойства:

  - `route`: начальный маршрут или `false`, чтобы пропустить начальную смену маршрута (по умолчанию `/`).

#### `mockNuxtImport`

`mockNuxtImport` подменяет автоимпорты Nuxt. Например, подмена `useState`:
...
Мок можно явно типизировать для типобезопасности и использовать оригинальную реализацию, передаваемую в фабрику, при подмене сложной логики.
...
Если нужно подменять поведение только внутри одного теста, можно использовать такой подход.
...
У объекта доступны следующие свойства:

- `handler`: функция-обработчик события
- `method`: (необязательно) HTTP-метод для сопоставления (например, 'GET', 'POST')
- `once`: (необязательно) если `true`, обработчик будет использован только для первого подходящего запроса и затем автоматически удалён
