...
- **Маршрутизация с учётом регистра**: маршруты страниц [совпадают с регистром URL](/docs/4.x/getting-started/upgrade#case-sensitive-routing), как в Nitro
...
- **Строгие side-effect импорты**: сгенерированный `tsconfig.json` включает [`noUncheckedSideEffectImports`](/docs/4.x/getting-started/upgrade#stricter-side-effect-imports) в соответствии с дефолтом TypeScript 7
- **Vue Options API отключён**: [Options API исключается из клиентского бандла](/docs/4.x/getting-started/upgrade#vue-options-api-disabled-by-default), что уменьшает его размер
...
### Маршрутизация с учётом регистра

🚦 **Уровень влияния**: Минимальный

#### Что изменилось

При `compatibilityVersion: 5` маршруты страниц сопоставляются с URL с учётом регистра, как в Nitro. Например, `/About` больше не совпадает с `pages/about.vue`.

#### Шаги миграции

Обновите ссылки, чтобы регистр совпадал с маршрутами страниц. Чтобы сохранить сопоставление без учёта регистра:
...
### Более строгие side-effect импорты

🚦 **Уровень влияния**: Минимальный

#### Что изменилось

При `compatibilityVersion: 5` сгенерированный Nuxt `tsconfig.json` включает [`noUncheckedSideEffectImports`](https://www.typescriptlang.org/tsconfig/#noUncheckedSideEffectImports). Это дефолт TypeScript 7, поэтому раннее включение выравнивает проект под будущее обновление.

С этой опцией side-effect import (`import './setup'`), который TypeScript не может сопоставить с модулем, становится ошибкой типов; раньше такой import игнорировался. Это касается только проверки типов (`nuxt typecheck` и редактор), не поведения в рантайме.

#### Зачем это сделано

Неразрешённые side-effect импорты молча пропускались: опечатка или удалённый файл могли пройти typecheck. Явная ошибка ловит такие случаи и совпадает с дефолтом TypeScript 7.

#### Шаги миграции

Если typecheck падает на side-effect import не-кодового ресурса (например `import '~/assets/styles.css'`), добавьте ambient-декларацию модуля, чтобы TypeScript считал import допустимым:
...
::tip
Чтобы вернуть прежнее поведение, отключите опцию в `nuxt.config`:
...
### Vue Options API отключён по умолчанию

🚦 **Уровень влияния**: Минимальный

#### Что изменилось

При `compatibilityVersion: 5` Nuxt выставляет флаг Vue [`__VUE_OPTIONS_API__`](https://vuejs.org/api/compile-time-flags#VUE_OPTIONS_API) в `false`, и рантайм Options API не попадает в клиентский бандл.

#### Зачем это сделано

Рантайм Options API попадает в каждый клиентский бандл, хотя большинство приложений Nuxt пишут на Composition API и `<script setup>`. Без него бандл меньше (примерно 6 КБ minified / 2 КБ gzipped на минимальном приложении).

#### Шаги миграции

Если какие-то ваши компоненты (или компоненты зависимости) используют Options API (`export default { data() {}, methods: {}, ... }`), включите его в `nuxt.config`:
...
::note
`defineNuxtComponent` не затронут: опции `asyncData` и `head` обрабатываются через `setup()`, а не через Vue Options API, поэтому работают независимо от этого флага.
::
