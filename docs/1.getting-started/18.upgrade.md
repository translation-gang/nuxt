...
Чтобы обновить Nuxt до [последнего релиза v3](https://github.com/nuxt/nuxt/releases), используйте команду `nuxt upgrade` с флагом `--channel=v3`.
...
::note
Команда сработает только если у вас уже стоит версия `@nuxt/cli`, в которой реализован флаг `--channel`. Если нет — для первого обновления используйте `nuxi@latest`, например: `npx nuxi@latest upgrade --dedupe --channel=v3`.
::
...
Чтобы использовать последнюю сборку Nuxt и тестировать функции до их выхода, см. [руководство по каналу ночных релизов](/docs/3.x/guide/going-further/nightly-release-channel).

::warning
Тег `latest` ночного канала сейчас следует за веткой Nuxt v4 — возможны частые ломающие изменения; будьте осторожны. Для ночных сборок ветки 3.x укажите `"nuxt": "npm:nuxt-nightly@3x"`.
::
...
Чтобы упростить обновление, мы сотрудничали с командой [Codemod](https://github.com/codemod/codemod) над автоматизацией шагов миграции открытыми codemod.

::note
При проблемах сообщите о них команде Codemod: `npx codemod feedback` 🙏
::

Полный список codemod для Nuxt 4, описание каждого, исходники и способы запуска — в [Codemod Registry](https://app.codemod.com/registry).
...
* появилась директория [`shared/`](/docs/3.x/directory-structure/shared) для кода, общего между Vue-приложением и сервером Nitro, с автоимпортом из `shared/utils/` и `shared/types/`
...
::note
При такой структуре алиас `~` по умолчанию указывает на каталог `app/` (ваш `srcDir`). То есть `~/components` ведёт в `components/`, `~/pages` — в `pages/` и т.д.
::
...
3. Убедитесь, что папки `nuxt.config.ts`, `content/`, `layers/`, `modules/`, `public/`, `shared/` и `server/` находятся вне папки `app/`, в корне вашего проекта.
...
Однако миграция _не обязательна_: при текущей раскладке каталогов Nuxt подстроится сам (если нет — [сообщите об issue](https://github.com/nuxt/nuxt/issues)). Исключение — если у вас уже задан свой `srcDir`: тогда `modules/`, `public/`, `shared/` и каталог [`server/`](/docs/3.x/directory-structure/server/) резолвятся от `rootDir`, а не от вашего пользовательского `srcDir`. При необходимости переопределите `dir.modules`, `dir.public` и `serverDir`.
...
В [документации по миграции](/docs/3.x/migration/overview) — пошаговое сопоставление возможностей Nuxt 2 и Nuxt 3+ и рекомендации по переносу приложения.
...
Если вы предпочитаете постепенно переводить приложения Nuxt 2 на Nuxt 3, используйте Nuxt Bridge — слой совместимости, который подключает возможности Nuxt 3+ в Nuxt 2 по мере необходимости. Подробнее — [документация Nuxt Bridge](/docs/3.x/bridge/overview).
