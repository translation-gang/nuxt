...
export default defineNuxtConfig({
  ...
  srcDir: '.',
  // Это определяет префикс директории для `router.options.ts` и `spa-loading-template.html`.
  dir: {
    app: 'app'
  }
})
...
При рендеринге страницы, предназначенной только для клиента (с `ssr: false`), мы опционально ренденрим экран загрузки (из `~/app/spa-loading-template.html` - note that this has also changed to `~/spa-loading-template.html` in Nuxt 4) в корне приложения Nuxt:
