...
Просмотрите issue и обсуждения проекта, которому хотите помочь. Для Nuxt: [issues](https://github.com/nuxt/nuxt/issues) и [discussions](https://github.com/nuxt/nuxt/discussions). Ответы другим, обходные пути, воспроизведение бага кодом или короткое расследование с выводами — всё это очень ценно.

### Создание issue

Спасибо, что нашли время оформить задачу! ❤️

* **Сообщения об ошибках**: сначала прочитайте [руководство](/docs/3.x/community/reporting-bugs), затем открывайте issue.

* **Запросы на улучшение**: убедитесь, что нет подходящего issue или обсуждения. Если идея относится к другой части экосистемы (например, к модулю), сначала подайте запрос туда. Если задумка общая или API ещё неясен, начните с обсуждения в категории **Ideas** (идеи) на GitHub.

Мы стараемся следовать [внутренней схеме разбора issue](https://mermaid.live/view#pako:eNqFlE1v2zAMhv8K4UuToslhx2Bo0TZt12Edhm7YMCAXWqJtorLk6qOpkfS_j7KdfpyWQ-BQr8mHL6nsCuU0FauiMm6rGvQRfq03FuRzvvvTYIQHthpcBT_ugQNwPHuZjheLxf4i1VDx8x4udrf5EBCOQvSsYg4ffS79KS9pmX9QALTgyid2KYB7Ih-4bmKWbDk2YB0E1gRUVaRi-FDmmjAmT3u4nB3DmoNKIUA1BsGSohA49jnVMQhHbDh_EZQUImyxh-gAtfaiG-KWSJ-N8nt6YtpCdgEeE5rXPOdav5YwWJIJU7zrvNADV9C7JBIyIC07Wxupkx3LFQ5vCkguRno5f9fP2qnUko0Y2dk9rGdvHAa9IIhVGlCp5FFNPN-ce4DKeXBd53xMliOLp9IZtyORQVsnrGm-WJzejtUu5fFqdr5FGQ3bLslYvGthjZbJTLpReZG5_lLYw7XQ_CbPVT92ws9gnEJj-v84dk-PiaXnmF1XGAaPsOsMKywNvYmG80ZohV8k4wDR9_N3KN_dHm5mh1lnkM5FsYzRfNiTvJoT5gnQsl6uxjqXLhkNQ9syHJ0UZZ8ERUIlNShr6N8gZDEliR-ow7QZa0fhY4LoHLRo-8N7ZxPwjRj5ZZYXpvOSNs9v3Jjs8NXB4ets92xan3zydXZHvj64lKMayh4-gZC1bjASW2ipLeWuzIuToiXfImu5rbucclMIc0ubYiWPGv3DptjYF9Fhiu5nb1Wxij7RSZE6jZHWjLXHtlhVaIJESXN0_m68_sO_wMs_oO9gyg).
...
Перед исправлением бага проверьте, нет ли **уже issue с описанием**. Возможно, это вопрос документации или есть контекст, полезный для фикса.

Если вы делаете новую возможность, **сначала откройте issue с запросом на функцию**, чтобы с сопровождающими согласовать цель и дизайн. Так экономится время и у сопровождающих, и у авторов, а релизы выходят быстрее. Issue **должен быть подтверждён** кем-то из команды фреймворка, прежде чем в PR появится реализация.
...
Как создать pull request на GitHub, описано в [официальной справке](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/proposing-changes-to-your-work-with-pull-requests/creating-a-pull-request).

Заголовок PR тоже должен соответствовать [конвенции коммитов](#конвенции-о-коммитах).

Если PR закрывает существующие issue, укажите их в описании.

В одном PR может быть несколько коммитов: **rebase** и **force push** не обязательны — при слиянии мы обычно выбираем **Squash and merge** (склеивание в один коммит) в интерфейсе GitHub.
...
Метка **pending** — внутренняя пометка команды (часто есть ещё шаги до финального ревью), она не про качество идеи. Причину по возможности поясним в комментарии.
...
Если модуль уже есть или он не подходит, откройте обсуждение: чётко опишите замысел, приложите примеры кода или сигнатуры для новых API, ссылки на связанные issue и болевые точки.
...
Документация — важная часть Nuxt. Мы стремимся быть интуитивно понятным фреймворком — и большая часть этой цели в том, чтобы и опыт разработчиков, и документация были выверены во всей экосистеме. 👌
