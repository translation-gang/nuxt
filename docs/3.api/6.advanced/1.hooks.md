...
description: Nuxt предоставляет мощную систему хуков для расширения практически всех аспектов с помощью хуков.
---
...
`app:data:refresh`           | `keys?`             | Сервер и клиент | (internal)
`vue:setup`                  | -                   | Сервер и клиент | (internal)
...
`app:beforeMount`            | `vueApp`            | Клиент          | Вызывается перед установкой приложения, вызывается только на клиенте.
`app:mounted`                | `vueApp`            | Клиент          | Вызывается, когда приложение Vue инициализируется и устанавливается в браузер.
...
`app:templates`             | `app`                                                    | Вызывается во время генерации `NuxtApp`, чтобы позволить настраивать, модифицировать или добавлять новые файлы в каталог сборки (либо виртуально, либо записывая их в `.nuxt`).
...
`builder:watch`             | `event, path`                                            | Вызывается во время сборки в режиме разработки, когда наблюдатель обнаруживает изменения в файле или каталоге проекта.
...
`server:devHandler`         | `handler`                                                | Вызывается при регистрации middleware разработки на сервере Nitro dev.
...
## Хуки приложения Nitro (время выполнения, server-side)
...
`error`           | `error, { event? }`                        | Вызывается при возникновении ошибки.     | [error](https://github.com/unjs/nitro/blob/main/src/runtime/types.ts#L24), [event](https://github.com/unjs/h3/blob/f6ceb5581043dc4d8b6eab91e9be4531e0c30f8e/src/types.ts#L38)
