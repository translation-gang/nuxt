...
::note
`data`, `pending`, `status` и `error` являются экземплярами Vue refs, и они должны быть получены с помощью `.value` при использовании внутри `<script setup>`, в то время как `refresh`/`execute` является обычной функцией для повторной загрузки данных.
::
...
- `options`:
  ...
  - `getCachedData`: Предоставьте функцию, которая возвращает кэшированные данные. Возвращаемое значение _null_ или _undefined_ будет вызывать выборку. По умолчанию это: `key => nuxt.isHydrating ? nuxt.payload.data[key] : nuxt.static.data[key]`, которая кэширует данные, только если включено `payloadExtraction`.
  ...
  - `deep`: возвращать данные в глубокий ref-объект (по умолчанию это `true`). Можно установить значение `false`, чтобы возвращать данные в неглубоком объекте, что может повысить производительность, если ваши данные не нуждаются в глубокой реакции.
