...
::note{icon="i-lucide-info"}
**Установите Claude Code** — инструкции в [документации Anthropic](https://code.claude.com/docs/en/quickstart).
::
...
3. Измените файл `claude_desktop_config.json`, добавив конфигурацию MCP-сервера:
    ...
    ```
4. Перезапустите приложение Claude Desktop. MCP-сервер Nuxt должен зарегистрироваться.
...
Если вы уже настроили MCP-серверы в VS Code (для Agent замените ключ `servers` на `mcpServers`), можно использовать похожую конфигурацию для GitHub Copilot coding agent. Добавьте ключ `tools`, перечисляющий инструменты, доступные Copilot.

1. Перейдите в репозиторий на GitHub
2. Откройте **Settings** → **Code & automation** → **Copilot** → **Coding agent**
3. В разделе **MCP configuration** добавьте конфигурацию:
    ```json
    {
      "mcpServers": {
        "nuxt": {
...
3. В PR нажмите **View session** в событии временной шкалы «Copilot started work»
4. Нажмите кнопку с многоточием (**...**) в правом верхнем углу, затем откройте **Copilot** в боковой панели
5. Разверните шаг **Start MCP Servers**, чтобы увидеть настроенные инструменты Nuxt

Подробнее об использовании MCP с GitHub Copilot coding agent: [Extend coding agent with MCP](https://docs.github.com/en/copilot/how-tos/use-copilot-agents/coding-agent/extend-coding-agent-with-mcp).
