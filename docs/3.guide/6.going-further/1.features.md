...
::important
Эта опция конфигурации доступна в Nuxt v3.12+. Пока что версию совместимости нужно задавать в **каждом** слое, который переключается на поведение Nuxt 4. После выхода Nuxt 4 это требование будет снято.
::
...
Это включает для TypeScript режим разрешения модулей `Bundler` («сборщик») — рекомендуемую настройку
...
См. [исходный запрос на включение изменений в репозитории TypeScript](https://github.com/microsoft/TypeScript/pull/51669).
