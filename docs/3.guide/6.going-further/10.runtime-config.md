...
::warning
Значения переменных окружения автоматически приводятся к типам JavaScript через [`destr`](https://github.com/unjs/destr). Например, `NUXT_MY_VAR=4848e0` станет числом `4848`. Чтобы сохранить строку, в значении переменной должны быть литеральные двойные кавычки: в `.env` пишите `NUXT_MY_VAR='"4848e0"'`; при прямой установке (в shell, Dockerfile или панели хостинга) убедитесь, что кавычки входят в значение и shell их не снимает (например, `NUXT_MY_VAR='"4848e0"' node .output/server/index.mjs`).
::
