...
Подробнее об извлечении payload и последствиях каждого режима.
::

## clientNodePlaceholder

Вместо элементов `<div>` в качестве placeholder для client-only компонентов при SSR используются comment nodes (`<!--placeholder-->`).

При включении компоненты `.client.vue` и обёртки `createClientOnly()` рендерят HTML-комментарий на сервере вместо пустого `<div>`. Это исправляет проблему гидрации Vue, когда scoped-стили могут не применяться, если placeholder `<div>` и корень компонента имеют одинаковое имя тега.

::warning
При включении атрибуты (`class`, `style` и т.д.), переданные компонентам `.client.vue`, не попадут в SSR HTML. Если нужны стилизованные placeholder, чтобы избежать layout shift, используйте `<ClientOnly>` со слотом `#fallback`.
::

Флаг включается при `future.compatibilityVersion: 5` или выше, но его можно включить явно:
...
::note
Пропускайте `nuxt-client` для компонентов без SFC, например [`<NuxtLink>`](/docs/4.x/api/components/nuxt-link). См. [Клиентские компоненты внутри серверных](/docs/4.x/directory-structure/app/components#client-components-within-server-components).
::
...
`pendingWhenIdle` управляет ref `pending`, который возвращают `useAsyncData` и `useFetch`.

При `pendingWhenIdle: false` (по умолчанию) `pending` равен `true`, пока запрос выполняется, и совпадает с `status === 'pending'`. Пока `status` — `idle`, `pending` остаётся `false`. Это видно с `{ immediate: false }` или с `{ server: false }` при серверном рендеринге.

Задайте `pendingWhenIdle: true`, чтобы `pending` был `true` и когда `status` — `idle`, а кэшированных данных нет. Используйте `status`, если UI загрузки должен различать `idle` и выполняющийся запрос.
...
::warning
**Статус и заголовки ответа нужно задать до отправки оболочки.** Стриминг фиксирует HTTP-статус и заголовки уже с первым байтом, поэтому всё, что меняет ответ позже, до клиента не доходит. Это свойство самого стриминга, а не баг Nuxt.

Граница — момент отправки оболочки:
