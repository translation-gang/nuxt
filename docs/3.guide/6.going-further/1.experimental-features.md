...
Внутри Nuxt использует `@nuxt/schema` для определения этих экспериментальных возможностей. За дополнительной информацией обратитесь к [документации API](/docs/3.x/api/nuxt-config#experimental) или см. [Исходный код](https://github.com/nuxt/nuxt/blob/main/packages/schema/src/config/experimental.ts).
...
В Nuxt и Nitro включён доступ к нативному асинхронному контексту для вложенных композаблов. Так можно вызывать композаблы из асинхронных композаблов и реже получать ошибку о недоступности экземпляра Nuxt.
...
При сборке выносит `vue`, `@vue/*` и `vue-router` за пределы клиентского бандла (как внешние зависимости).
...
Это может заметно ускорить пререндеринг сайтов, где `useAsyncData` или `useFetch` и
...
Это работает только со статическими данными или строками/массивами, а не с переменными или условными присваиваниями. Дополнительный контекст — в [задаче на GitHub](https://github.com/nuxt/nuxt/issues/24770).
