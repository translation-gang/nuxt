...
Полностью выведенный тип доступен только в коде приложения (компоненты, composables, плагины и т.д.). В server routes, коде в директории `shared/` и `nuxt.config` ключи из файлов `app.config` типизируются как `unknown`: типизация там потребовала бы проверки типов кода приложения вне app-контекста, где автоимпорты недоступны. Ключи, заданные inline в опции `appConfig` в `nuxt.config`, типизируются везде. Если нужны другие ключи вне app-кода, можно расширить интерфейс `AppConfig`, как описано ниже. Расположение файла augmentation определяет, в каких контекстах он виден: `.d.ts` в `shared/` покрывает app code, shared code и server routes.

Тем не менее есть случаи, когда типы нужно задать вручную. Есть два возможных варианта типизации.
