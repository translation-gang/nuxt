...
Чтобы группировать компоненты в каталоге, не меняя их имени, можно использовать круглые скобки `(` `)` в имени группирующей директории.
...
…то имя компонента пропустит группирующую директорию и будет таким:
...
Каждая запись каталога также принимает glob-опции `pattern` и `ignore`, которые управляют тем, какие файлы сканируются внутри `path`. Это полезно, когда компоненты лежат во вложенных папках с нестандартной структурой, например в domain-driven архитектуре:
...
::note
Если указан `pattern`, опция `extensions` не действует — убедитесь, что pattern совпадает с расширениями файлов, которые нужно сканировать.
::
...
::warning
Используйте `nuxt-client` только на локальных SFC `.vue`. Встроенные компоненты вроде [`<NuxtLink>`](/docs/4.x/api/components/nuxt-link) не проходят islands transform. После клиентской навигации может появиться `Failed to locate Teleport target`, или ссылка исчезнет без ошибки. Оберните встроенный компонент в свой `.vue` и поставьте `nuxt-client` на обёртку, либо уберите `nuxt-client` со встроенного компонента. См. [#29251](https://github.com/nuxt/nuxt/issues/29251) и [#26002](https://github.com/nuxt/nuxt/issues/26002).
::
...
::important
`useRoute()` и другие композаблы `vue-router` не отслеживают текущий маршрут страницы внутри серверного (островного) компонента. Остров рендерится в отдельном изолированном Vue-приложении, привязанном только к своим props (и явному контексту) — поэтому острова можно кэшировать независимо от страницы, на которой они отображаются. Внутри острова `useRoute()` отражает запрос самого острова, а не страницу пользователя.

Если острову нужна информация о текущем маршруте, передайте её явно — через props из родительского компонента или через prop `context` у `<NuxtIsland>` (внутри острова читается из `nuxtApp.ssrContext.islandContext`).
::
