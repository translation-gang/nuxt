...
## Передача пропсов в лейауты :badge[+4.4]{color="primary" class="align-middle"}
...
::tip
Пропсы полностью типизированы на основе `defineProps` вашего лейаута. В редакторе будут автодополнение и проверка типов.
::

### Через `setPageLayout`

Пропсы можно передать и при смене лейаута через [`setPageLayout`](/docs/4.x/api/utils/set-page-layout):
