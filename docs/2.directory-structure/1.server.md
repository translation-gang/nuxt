...
Обработчик может возвращать JSON, `Promise` или объект `Response`.

::important
Не импортируйте код Vue-приложения (компоненты, композаблы и другие утилиты только для app) в серверные маршруты или утилиты, и не импортируйте серверный код в app.
::
