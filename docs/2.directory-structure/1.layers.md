...
Либо укажите каталоги в [`extends`](/docs/4.x/api/nuxt-config#extends) (например, `extends: ['~~/layers/admin', '~~/layers/base']`), чтобы задать порядок из `nuxt.config` без переименования; первая запись имеет наивысший приоритет.
