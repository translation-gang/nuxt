...
::note
Несмотря на название «страница ошибок», это не маршрут: файл не кладут в `~/pages` и не используют на нём `definePageMeta`. Лейаут по-прежнему можно подключить через [`NuxtLayout`](/docs/3.x/api/components/nuxt-layout), указав имя лейаута.
::
