...
::important
`useRoute()` и другие композаблы `vue-router` не отслеживают текущий маршрут страницы внутри серверного (островного) компонента. Остров рендерится в собственном изолированном Vue-приложении, ключом которого служат только его props (и любой явный контекст) — благодаря этому острова можно кэшировать независимо от страницы, на которой они отображаются. Внутри острова `useRoute()` будет отражать собственный запрос острова, а не страницу пользователя.

Если острову нужна информация о текущем маршруте, передайте её явно — через props из родительского компонента или через prop `context` в `<NuxtIsland>` (внутри острова читается из `nuxtApp.ssrContext.islandContext`).
::

Внутри островного компонента контекст острова доступен через `nuxtApp.ssrContext.islandContext`. Обратите внимание: пока островные компоненты помечены как экспериментальные, формат этого контекста может измениться.
