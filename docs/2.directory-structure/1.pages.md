...
::tip
Страницы попадают в пререндер автоматически только если `nitro.prerender.crawlLinks` не отключён и в `nitro.prerender.routes` есть хотя бы один маршрут. Подробнее — в [руководстве по пререндерингу](/docs/3.x/getting-started/prerendering).
::
