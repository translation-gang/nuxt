...
::note
Относительные пути (например `./`) нельзя задавать напрямую в `nuxt.config.ts` из‑за ограничений Nitro. Для статического хостинга с относительными путями к ресурсам используйте один из вариантов:

```bash
# Вариант 1: переменная окружения на этапе сборки
NUXT_APP_BASE_URL=./ npm run generate
```

```ts
// Вариант 2: runtime config Nitro
export default defineNuxtConfig({
  nitro: {
    runtimeConfig: {
      app: {
...
**См.**: [Nuxt View Transition API docs](/docs/4.x/getting-started/transitions#view-transitions-api-experimental)
...
**См.**: [`app/components/` directory documentation](/docs/4.x/directory-structure/app/components)
...
**См.**: [Nuxt documentation](/docs/4.x/directory-structure/app/composables)
...
**См.**: [`app/plugins/` directory documentation](/docs/4.x/directory-structure/app/plugins)
...
**См.**: [`server/` directory documentation](/docs/4.x/directory-structure/server)
...
- Если не задано, используется файл `~/spa-loading-template.html` из одного из слоёв (если есть). — Если `false`, индикатор загрузки SPA не показывается. — Если `true`, Nuxt ищет файл `~/spa-loading-template.html` в слоях; при отсутствии подставляется изображение по умолчанию.
...
Расширяет сгенерированную конфигурацию TypeScript `.nuxt/tsconfig.app.json` (и устаревший `.nuxt/tsconfig.json`). Опции здесь имеют приоритет над `tsConfig`.

**См.**: [tsconfig.json information](/docs/4.x/directory-structure/tsconfig)
...
Расширяет сгенерированную конфигурацию TypeScript `.nuxt/tsconfig.node.json`.

**См.**: [tsconfig.json information](/docs/4.x/directory-structure/tsconfig)

### `serverTsConfig`

Расширяет сгенерированную конфигурацию TypeScript `.nuxt/tsconfig.server.json`. Опции здесь имеют приоритет над `tsConfig`.

**См.**: [tsconfig.json information](/docs/4.x/directory-structure/tsconfig)

### `sharedTsConfig`

Расширяет сгенерированную конфигурацию TypeScript `.nuxt/tsconfig.shared.json`.
...
В TypeScript доступны дополнительные проверки для повышения надёжности и анализа кода. После перехода кодовой базы на TypeScript можно постепенно включать эти проверки. [Подробнее](https://www.typescriptlang.org/docs/handbook/migrating-from-javascript.html#getting-stricter-checks)
...
Вы можете расширить сгенерированные tsconfig с общими опциями через эту настройку. `compilerOptions` применяются ко всем сгенерированным tsconfig (`.nuxt/tsconfig.app.json`, `.nuxt/tsconfig.server.json`, `.nuxt/tsconfig.node.json` и `.nuxt/tsconfig.shared.json`), а `include`, `exclude` и `vueCompilerOptions` — только к `.nuxt/tsconfig.app.json` (и устаревшему `.nuxt/tsconfig.json`). Две группы `compilerOptions` — исключения: DOM- и Vue-специфичные опции (например, `lib`, `jsx` и `jsxImportSource`) применяются только к `.nuxt/tsconfig.app.json`, а `types`, `paths` и `noEmit` управляются Nuxt для каждого контекста, поэтому их нельзя задавать глобально для tsconfig `node`, `shared` и `server`. Используйте `appTsConfig`, `serverTsConfig`, `nodeTsConfig` или `sharedTsConfig` для переопределений по контексту; они имеют приоритет над этой опцией.
...
**См.**: [Nuxt TypeScript docs](/docs/4.x/guide/concepts/typescript)
...
Включает режим совместимости с предыдущими версиями модуля `unhead`. При этом: отключается сортировка Capo.js; добавляется `DeprecationsPlugin` (поддержка `hid`, `vmid`, `children`, `body`); добавляется `PromisesPlugin` (поддержка промисов на входе).
...
Опции `vite` верхнего уровня общие для клиентского и серверного окружений. Используйте `$client` и `$server`, чтобы задать конфигурацию для конкретного окружения — она будет объединена с соответствующей сборкой.
...
Конфигурация, объединяемая с настройками Vite для клиентской (браузерной) сборки.

- **Тип**: `object`

### `$server`

Конфигурация, объединяемая с настройками Vite для серверной сборки.
...
Включает реактивный деструктуринг для `defineProps`.
...
Разделение на несколько CSS-файлов лучше для кэширования и изоляции preload: браузер скачивает и обрабатывает только нужные ресурсы, что может ускорить загрузку страницы.
