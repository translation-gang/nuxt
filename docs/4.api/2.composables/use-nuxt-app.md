...
`useNuxtApp` — встроенный композабл для доступа к общему контексту выполнения Nuxt ([контекст Nuxt](/docs/3.x/guide/going-further/nuxt-app#the-nuxt-context)) на клиенте и сервере (но не в маршрутах Nitro). Через него доступны экземпляр Vue-приложения, runtime-хуки, `runtimeConfig` и внутреннее состояние вроде `ssrContext` и `payload`.
...
Если контекст выполнения недоступен, `useNuxtApp` выбросит исключение. Для проверки без ошибки или для композаблов, где `nuxtApp` не обязателен, используйте [`tryUseNuxtApp`](/docs/3.x/api/composables/use-nuxt-app#tryusenuxtapp).
...
- `data` (`object`) — при загрузке через [`useFetch`](/docs/3.x/api/composables/use-fetch) или [`useAsyncData`](/docs/3.x/api/composables/use-async-data) результат попадает в `payload.data`; записи кэшируются, чтобы не дублировать одинаковые запросы.
  ...
  Обращаясь к тем же `payload.data` из [`ssrContext`](/docs/3.x/api/composables/use-nuxt-app#ssrcontext), на сервере вы получите то же значение.
...
#### Пользовательский редьюсер / ревайвер
  ...
  С [Nuxt v3.4](https://nuxt.com/blog/v3-4#payload-enhancements) можно задать свой редьюсер и ревайвер для типов, которые Nuxt не сериализует по умолчанию.

  :video-accordion{title="Видео Александра Лихтера о сериализации payload, в том числе для классов" videoId="8w6ffRBs8a4"}
