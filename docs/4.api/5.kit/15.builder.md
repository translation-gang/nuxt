...
**`options`**: опции, передаваемые в колбэк. Объект может содержать следующие свойства:
...
Задаёт провайдер build output для указанного ключа. Build outputs — контракт между сборщиками (Vite, webpack, Rspack или кастомным builder) и серверным рантаймом Nitro: каждый ключ соответствует импорту подпути `nuxt/*`, который серверный рантайм разрешает на этапе сборки. Обычно вызывается изнутри builder или из модуля, участвующего в сборке.

::read-more{to="/docs/4.x/guide/going-further/builders#the-build-output-contract"}
Подробнее о контракте build output.
::
...
**`key`**: ключ build output. Один из `serverEntry`, `clientManifest`, `clientPrecomputed`, `ssrStyles`, `entryChunkName` или `entryIds`.

**`provider`**: значение для этого ключа — (возможно асинхронная) функция, возвращающая тело модуля в виде строки; читается лениво, когда серверная сборка разрешает соответствующий импорт `nuxt/*`.
