...
| `meta`     | `NuxtPageMeta`{lang="ts"}          | нет          | Пользовательские метаданные маршрута (макеты, middleware, навигационные гарды). |
...
### Типизация пользовательских метаданных страниц

Интерфейс `NuxtPageMeta` можно расширить, чтобы добавить типобезопасные пользовательские метаданные страниц при использовании `extendPages` или хука `pages:extend`:
  ...
  setup () {
    extendPages((pages) => {
      for (const page of pages) {
        // requiresAuth теперь типизирован как boolean | undefined
...
**options**: объект с конфигурацией маршрута. При `override: true` перезаписывает существующую конфигурацию маршрута.
