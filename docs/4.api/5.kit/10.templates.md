...
| `getContents`| `(data: Options) => string \| Promise<string>`{lang="ts"} | нет          | Функция, вызываемая с объектом `options`; возвращает строку или Promise. При заданном `src` не используется.                                     |
