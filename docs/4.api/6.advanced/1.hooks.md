...
| `page:loading:start`         | -                   | Client          | Вызывается при начале навигации по маршруту (до разрешения) или при смене ключа страницы. Может сработать без повторного запуска `setup()` компонента страницы, если страница переиспользуется (например, со статическим `key` в `definePageMeta`). |
