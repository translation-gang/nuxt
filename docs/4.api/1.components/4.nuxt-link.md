...
При ссылке на другое приложение на том же домене использование пропа `external` обеспечивает корректное поведение.
...
При использовании пропа `custom` `<NuxtLink>` не подключает обработчики prefetch автоматически. Вызовите `prefetch` из custom-слота самостоятельно.
...
При использовании `custom` prefetch контролируется реализацией слота :badge[v4.5]{color="info" size="xs" class="align-middle"}:
...
::note
При пропе `custom` `prefetch`, `prefetchOn` и `prefetchedClass` не подключают обработчики и классы автоматически. Используйте значения `prefetch`, `prefetched` и `shouldPrefetch` из custom-слота, чтобы реализовать это поведение.
::
...
- `componentName`: внутреннее имя компонента, как в Vue DevTools. Не меняет имя в шаблонах (оно берётся из имени файла компонента). По умолчанию `NuxtLink`.
- `externalRelAttribute`: значение атрибута `rel` для внешних ссылок. По умолчанию `"noopener noreferrer"`. Пустая строка `""` отключает.
- `activeClass`: класс для активной ссылки. Аналог [Vue Router `linkActiveClass`](https://router.vuejs.org/api/interfaces/routeroptions#linkActiveClass-). По умолчанию значение Vue Router (`"router-link-active"`).
- `exactActiveClass`: класс для точного совпадения. Аналог [Vue Router `linkExactActiveClass`](https://router.vuejs.org/api/interfaces/routeroptions#linkExactActiveClass-). По умолчанию значение Vue Router (`"router-link-exact-active"`).
