...
В API-маршрутах лучше передавать объект с кратким `statusText`: он доступен на клиенте. `message`, переданный в `createError` в API, на клиент не попадает. Дополнительные данные можно передать через свойство `data`. При обработке ошибки через `useFetch` пользовательские данные доступны в `error.value.data.data`. Не подставляйте в сообщение динамический пользовательский ввод — это может быть небезопасно.
