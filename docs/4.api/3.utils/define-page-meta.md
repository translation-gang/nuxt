...
- **тип**: `PageMeta`
  ...
  **`scrollToTop`**
  ...
  - **тип**: `boolean | (to: RouteLocationNormalizedLoaded, from: RouteLocationNormalizedLoaded) => boolean`
    ...
    Включить или отключить прокрутку вверх перед рендером страницы. Навигация не зависит от рендера, поэтому прокрутка срабатывает даже без перерисовки (например, при фиксированном [`key`](/docs/3.x/api/utils/define-page-meta#key)). Чтобы отключить её в таких случаях, задайте `scrollToTop: false`. Своё поведение прокрутки настройте в `~/app/router.options.ts` — см. [кастомную маршрутизацию](/docs/3.x/guide/recipes/custom-routing#using-routeroptions).
