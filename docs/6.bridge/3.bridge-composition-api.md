...
- `withContext` был удалён. См. [ниже](/docs/3.x/bridge/nuxt3-compatible-api#usecontext-and-withcontext).
- `useStatic` удалён, прямой замены нет. Если сценарий с `useStatic` важен — опишите его в обсуждении на GitHub.
- `reqRef` и `reqSsrRef`, которые были устаревшими, теперь полностью удалены. Следуйте инструкциям ниже относительно [ssrRef](/docs/3.x/bridge/nuxt3-compatible-api#ssrref-and-shallowssrref), чтобы заменить его.
...
Вы также можете продолжать использовать плагины в стиле Nuxt 2, удалив функцию (как и в случае с [`defineNuxtMiddleware`](/docs/3.x/bridge/bridge-composition-api#definenuxtmiddleware)).
...
Nuxt Bridge предоставляет прямые замены для этих композаблов через [`useRouter`](/docs/3.x/api/composables/use-router) и [`useRoute`](/docs/3.x/api/composables/use-route).
