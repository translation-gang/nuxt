...
Чтобы избежать ошибок при увлажнении, он будет применяться только после как приложение Vue было смонтировано, что означает, что при первоначальной загрузке может наблюдаться мерцание.
