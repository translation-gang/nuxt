...
Вызывает хук `app:chunkError`, когда происходит ошибка загрузки фрагментов vite/webpack. Поведение по умолчанию — выполнить перезагрузку нового маршрута при переходе на новый маршрут, когда фрагмент не загружается.
