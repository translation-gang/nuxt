...
### Использование стартового модуля

Узнайте, как выполнять основные задачи с помощью стартового модуля.
...
- Затем, если набор тестов прошел успешно, модуль будет опубликован:
  - Повышение версии модуля и создание журнала изменений в соответствии с вашими конвенциональными коммитами
...
Модули, как и все остальное в конфигурации Nuxt, не включаются во время выполнения вашего приложения. Тем не менее вы можете захотеть, чтобы ваш модуль предоставлял или внедрял код времени выполнения в приложение, на которое он установлен. Именно это и позволяет сделать runtime-директория.
...
Поскольку модули не являются частью времени выполнения приложения, их опции также не являются таковыми. Тем не менее во многих случаях вам может понадобиться доступ к некоторым из этих опций модуля в вашем коде времени выполнения. Мы рекомендуем "открывать" нужный конфиг с помощью Nuxt'овского [`runtimeConfig`](/docs/api/nuxt-config#runtimeconfig).
...
[Ознакомьтесь с этим RFC, чтобы присоединиться к разговору](https://github.com/nuxt/nuxt/discussions/18399).
::
...
Наличие приложения Nuxt для тестирования модуля при его разработке очень полезно. [Стартовый модуль интегрирует одно из них для этой цели](#how-to-develop).
...
Хорошей практикой является создание минимальной репродукции с вашим модулем и [StackBlitz](https://nuxt.new/s/v3), которую вы добавляете в README вашего модуля.
