...
Если вы размещаете приложение на [статичном хостинге](/docs/getting-started/deployment#static-hosting) с помощью команд `nuxt generate` или `nuxt build --prerender`, то по умолчанию Nuxt будет рендерить каждую страницу как отдельный статичный HTML-файл.
...
Nuxt-сервер будет автоматически регистрировать соответствующие middleware и оборачивать маршруты обработчиками кэша, используя [слой кеширования Nitro](https://nitro.build/guide/cache).
...
Пограничный рендеринг возможен, благодаря [Nitro](https://nitro.build/) - [серверному движку](/docs/guide/concepts/server-engine) - который обеспечивает работу Nuxt. Он предлагает кроссплатформенную поддержку Node.js, Deno, Cloudflare Workers и других.
