...
description: "Каталог components/ - это место, куда вы поместите все ваши компоненты Vue."
...
При этом компоненты регистрируются с использованием той же стратегии, что и в Nuxt 2. Например, `~/components/Some/MyComponent.vue` будет использоваться как `<MyComponent>` но не `<SomeMyComponent>`.
