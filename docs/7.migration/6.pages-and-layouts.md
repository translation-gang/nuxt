...
Создайте `app.vue` и перенесите в него логику, которая должна выполняться один раз на верхнем уровне приложения. Пример — в [справочнике по `app.vue`](/docs/3.x/directory-structure/app).

## Лейаут

Если вы используете лейауты в своем приложении для нескольких страниц, требуется внести незначительные изменения.

В Nuxt 2 текущую страницу в лейауте выводит `<Nuxt>`. В Nuxt 3 лейауты строятся на слотах — замените его на `<slot />`. Так проще делать именованные слоты и слоты с областью видимости (scoped). [Подробнее о лейаутах](/docs/3.x/directory-structure/layouts).
