...
Nuxt 3 является [родным фреймворком ESM](/docs/guide/concepts/esm). Хотя [`unjs/jiti`](https://github.com/unjs/jiti) обеспечивает частичную совместимость при загрузке файла `nuxt.config`, избегайте любого использования `require` и `module.exports` в этом файле.
